...
public class FreeBoardDAO {
	...
	//좋아요 등록
		...
		public int[] getPreOrNextBoard(int free_num)throws Exception{
			Connection conn = null;
			PreparedStatement pstmt = null;
			ResultSet rs = null;
			String sql = null;
			...
			try {
				conn = DBUtil.getConnection();
				sql = "select lag, lead from (select free_num,lag(free_num,1,0) over (order by free_num) as lag, lead(free_num,1,0) over (order by free_num) as lead from free_board) where free_num=?";
				pstmt = conn.prepareStatement(sql);
				pstmt.setInt(1, free_num);
				rs = pstmt.executeQuery();
				if(rs.next()) {
